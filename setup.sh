...
print_summary() {
  ...
  echo -e "    sam build"
  echo -e "    sam local start-api --env-vars env.json --port 3001"
  echo -e "    # In another terminal:"
  echo -e "    echo 'REACT_APP_API_URL=http://localhost:3001' > frontend/.env.local"
  echo -e "    cd frontend && npm start"
  echo ""
  echo -e "  ${YELLOW}Deploy to AWS — via GitHub Actions (recommended):${NC}"
  echo -e "    Push to main branch — CI/CD handles everything automatically."
  echo -e "    Required secrets: AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY,"
  echo -e "                      GEMINI_API_KEY, EXCHANGE_RATES_API_KEY, API_URL"
  echo ""
  echo -e "  ${YELLOW}Deploy to AWS — manual (first time):${NC}"
  echo -e "    sam build"
  echo -e "    sam deploy --guided"
  echo -e "    # Stack: uk-financial-comparison, Region: eu-west-2"
  echo ""
